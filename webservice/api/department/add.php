...
$departments->department_name = $post_data->department_name;

//auto gen infomation
if ($post_data->department_id == "auto_gen") {
  $departments->department_id = md5($post_data->department_name.date_timestamp_get($date));
} else {
...
if (verifyToken($user_id, $db, $token) == "Valid") {
  //run Code
  if ($departments->addDepartments() == "True") {
    echo json_encode(
      array(
        "msg" => "Department Was Succesfully Added"
      )
    );
  } else {
    echo json_encode(
      array(
        "msg" => "Department Was Unsuccesfully Added"
      )
    );
  }
} else {
